...
class kaznacheyLib
{
	...
	public	$kaznachey_currency;
	public	$http;
	
	public function __construct($processor_data=false)
	{
		if($processor_data){
			$this->merchantGuid = $processor_data['params']['kaznachey_merchantGuid'];
			$this->merchnatSecretKey = $processor_data['params']['kaznachey_merchnatSecretKey'];
			$this->kaznachey_currency = $processor_data['params']['kaznachey_currency']?$processor_data['params']['kaznachey_currency']:1;
		}
   ...
   function createOrder($order_id, $order_info, $selectedPaySystemId = false)
   {
	...
	function fn_kaznachey_currency ($price)
	{
		return number_format($price * $this->kaznachey_currency, 2, '.', '');
	}
